...
from force_estimation_net import ForcePredictionNetwork
...
def rmse(predicted_force, true_force):
    return torch.sqrt(torch.mean((predicted_force - true_force) ** 2))
...
def combined_loss(left_image, right_image, reconstructed_right, reconstructed_left, disparity, predicted_force, true_force):
    ...
    combined = reconstruction_loss + force_pred_loss
    return combined, reconstruction_loss, force_pred_loss
...
def main():
        ...
        for phase in ["train", "test"]:
            ...
            for left_image, right_image, true_force in tqdm(dataloaders[phase]):
                ...
                with torch.set_grad_enabled(phase == "train"):
                    ...
                    rmse_loss = rmse(predicted_force, true_force)
                    if phase == "train":
                        loss.backward()
                        optimizer.step()
    ...
    util.save_and_plot_losses(num_epochs, train_rmse,
                              test_rmse, "Force RMSE", "RMSE")
