...
class NetFactory(nn.Module):
    ...
    def input(self, in_dim=3, stride=1, activated=True):
        ...
        n = Node(inp=None, module=None, activated=activated,
                 stride=stride, dim=in_dim)
        self.nodes.append(n)
        return n
        ...
        for node in self.nodes:
            if node.module is None:  # initial input
                ...
                if node in self.skips:
                    x = outputs[self.skips[node]] + \
                        node.module(outputs[node.inp])
                else:
...
def hdrn_alpha_base(num_channels):
    ...
    x16 = x = net.block(x, '12591259')
    x = net.upsample(x, x16, num_channels // 2)
...
class StereoBackbone(nn.Module):
    ...
    def forward(self, stacked_img, step, robot_joint_angles=None):
        small_disp = self.stereo_stem.forward(
            stacked_img[:, 0:3], stacked_img[:, 3:6])
...
def dot_product_cost_volume(left, right, num_disparities: int, is_right: bool):
    ...
    for i in range(num_disparities):
        if not is_right:
            output[:, i, :, i:] = (left[:, :, :, i:] *
                                   right[:, :, :, :width - i]).mean(dim=1)
        else:
            output[:, i, :, width - i] = (left[:, :, :, i:]
                                          * right[:, :, :, :width - i]).mean(dim=1)
...
def main():
    ...
    onnx.save(shape_inference.infer_shapes(
        onnx.load('test_net.onnx')), 'test_net_shapes.onnx')
