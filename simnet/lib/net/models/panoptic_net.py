...
class PoseHead(nn.Module):
    ...
    def forward(self, features):
        z_centroid_output = self.z_centroid_head.forward(
            features).squeeze(dim=1)
...
class PanopticNet(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        self.seg_head = SegmentationHead(
            backbone_output_shape_4x, backbone_output_shape_8x, 5, hparams)
        # Add pose heads.
        self.pose_head = OBBHead(
            backbone_output_shape_4x, backbone_output_shape_8x, hparams)
    ...
    def forward(self, image, step):
        ...
        small_depth_output = depth_outputs.DepthOutput(
            small_disp_output, self.hparams.loss_depth_mult)
